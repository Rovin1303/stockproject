...
def get_gold_silver_correlation(period="5y", interval="1d"):
    gold = yf.download("GC=F", period=period, interval=interval, auto_adjust=False, progress=False)
    silver = yf.download("SI=F", period=period, interval=interval, auto_adjust=False, progress=False)
    ...
    # Price regression: Gold as a function of Silver for user-input prediction.
